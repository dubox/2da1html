    ...
    var g = {  //公共方法， 外部可调用
      //初始化
		init: function () {
			...
			silderPage.css('right',((wh.width-26*silderPage_li.size())/2)+'px');
			...
			silderPage_li.on('click',function(){
				pagesize = $(this).index();
				silderList_li.eq(pagesize).fadeIn(500).siblings().fadeOut(100);
			...
			//上一页
			$(opt.s_prev ).on('click',function(){
				//var pagesize = 0;
				pagesize = g.obj.find('.silder_list li:visible').index();
				if((--pagesize) < 0 ){
					pagesize = 0;
				}
				silderList_li.eq(pagesize).fadeIn(500).siblings().fadeOut(100);
			...
			//下一页
			$( opt.s_next).on('click',function(){
				//var pagesize = 0;
				
				pagesize = g.obj.find('.silder_list li:visible').index();
				
				if((++pagesize) >  silderList_li.size()){
					pagesize = silderList_li.size();
				}
